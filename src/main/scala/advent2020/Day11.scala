...
object Day11 {
  type InputType = Array[Array[SeatStatus.Value]]

  private def ready(input: String): InputType =
    input
      .linesIterator
      .map(_.toCharArray.map(SeatStatus(_)))
      .toArray

  def main(args: Array[String]): Unit = {
    val testData = time("testReady", () => ready(testInput))

    println(neighbors(0, 0)(testData))
    time("testSolve1", () => solve1(testData)) // should be 37
//    time("testSolve2", () => solve2(testData)) // should be 62
//
...
//    time("solve2", () => solve2(data))
  ...
  private def neighbors(i: Int, j: Int)(implicit data: InputType): Iterable[SeatStatus.Value] = {
    (Seq.empty :+
      at(i - 1, j - 1) :+
      at(i - 1, j)  :+
      at(i - 1, j + 1) :+
  ...
  private def at(i: Int, j:Int)(implicit data: InputType): Option[SeatStatus.Value] = {
    if (data.indices.contains(i) && data(0).indices.contains(j)) {
