...
object Day04 {
  ...
  type Passports = Array[Array[(String, String)]]
  ...
  val validators: Map[String, String => Boolean] = Map(
    ...
    ("iyr", s => "2010" <= s && "2020" >= s),
    ("eyr", s => "2020" <= s && "2030" >= s),
    ("hgt", _ matches "((1[5-8][0-9]|19[0-3])cm|(59|6[0-9]|7[0-6])in)"),
    ("hcl", _ matches "#[0-9|a-f]{6}"),
    ("ecl", _ matches "amb|blu|brn|gry|grn|hzl|oth"),
    ("pid", _ matches "[0-9]{9}"),
    ("cid", _ => true),
  )

  def ready(input: String): Array[String] = {
    input.linesIterator
      .map(_.trim)
      .toArray
  }
  ...
  def ready2(input: String): Passports = {
    ...
    val passports = lines
      ...
      .map(_.split(' ').map(_.trim).filter(_.nonEmpty)
        .map(t => {val a = t.split(':'); (a(0), a(1))}))
    passports
  }

  def main(args: Array[String]): Unit = {
    val testLines = ready(testInput)
    time("solve1 test", () => solve1(testLines))

    val lines = ready(input)
    time("solve1", () => solve1(lines))

    val passports = ready2(input)
    time("solve1_2", () => solve1_2(passports))
    time("solve2", () => solve2(passports))
  }

  def solve1(lines: Array[String]): Int = {
    var flags: Seq[String] = Seq()
    var valid: Int = 0
    for (line <- lines) {
      if (line.isEmpty) {
        valid += (
          if (mandatoryFields.forall(flags.contains(_))) 1
          else 0)
        flags = Seq()
      }
      val flagSet = line.split(' ').iterator
        .filter(_.nonEmpty).map(_.split(':')(0)).toSeq
      flags ++= flagSet
    }
    valid += (
      if (mandatoryFields.forall(flags.contains(_))) 1
  ...
  def solve1_2(passports: Passports): Int = {
    val valid = passports.count(passport => mandatoryFields.forall(term =>
      passport.map(_._1).contains(term)
    ))
    valid
  }

  def solve2(passports: Passports): Int = {
    val default: String => Boolean = _ => false
    val valid = passports
      .filter(passport => mandatoryFields.forall(term =>
        passport.map(_._1).contains(term)))
      .count(passport => passport.forall(term =>
        validators.getOrElse(term._1, default)(term._2)))
    valid
  }
