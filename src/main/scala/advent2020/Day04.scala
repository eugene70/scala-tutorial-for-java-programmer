...
object Day04 {
  ...
  def main(args: Array[String]): Unit = {
    val lines = ready(testInput)
    time("solve1", () => solve1(lines))
  }

  def solve1(lines: Array[String]): Unit = {
    var flags: List[(String, Boolean)] = mandatoryFields.map((_, false))
    var valid: Int = 0
    for (line <- lines) {
      if (line.isEmpty) {
        valid += (if (flags.forall(_._2)) 1 else 0)
        flags = mandatoryFields.map((_, false))
      }
      line.split(' ').iterator.foreach(flags. _.split(':')(0))
    }
    valid += (if (flags.forall(_._2)) 1 else 0)
    println(s"valid count: $valid")
  }
