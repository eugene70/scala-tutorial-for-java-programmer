...
object Day07 {
  private def ready(input: String): Map[String, Map[String, Int]] =
    input
      .replaceAll(""" bags\.| bags| bag\.| bag""", "")
      .linesIterator
      .map(_.trim)
      .filter(_.nonEmpty)
      .map(_.split(""" contain """))
      .filterNot("no other" == _(1))
      .map(a => (a(0), a(1).split(", ")))
      .map(a => (a._1, a._2.map(_.splitAt(2).swap).map(t => (t._1.trim, Integer.parseInt(t._2.trim))).toMap))
      //.map(a => {println(a); a})
      .toMap

  def main(args: Array[String]): Unit = {
    val testRules = time("testReady", () => ready(testInput))
    time("testSolve1", () => solve1(testRules)) // should be 4
    time("testSolve2", () => solve2(testRules)) // should be 32
    val testRules2 = time("testReady2", () => ready(testInput2))
    time("testSolve2", () => solve2(testRules2)) // should be 126

    val rules = time("ready", () => ready(input))
    time("solve1", () => solve1(rules))
    time("solve2", () => solve2(rules))
  }

  private def solve1(implicit rules: Map[String, Map[String, Int]]): Int =
    rules.count(m =>
      m._2.contains("shiny gold") ||
        m._2.keySet.exists(k => travers(k))
    )

  private def travers(bag: String)(implicit rules: Map[String, Map[String, Int]]): Boolean = {
    rules.get(bag).exists(m =>
      m.contains("shiny gold") ||
        m.keySet.exists(k => travers(k)))
  }

  private def solve2(implicit rules: Map[String, Map[String, Int]]): Int = travers2("shiny gold")

  private def travers2(bag: String)(implicit rules: Map[String, Map[String, Int]]): Int = {
    rules.get(bag)
      //.map(a => {println(a); a})
      .map(m => m.map(b => b._2 + b._2 * travers2(b._1)).sum)
      .sum
  }
