...
object Day08 {
  ...
  object Op extends Enumeration {
    ...
    implicit class Executor(op: Value) {
      def execute(acc: Int, line: Int, operand: Int): (Int, Int) = op match {
        case NOP => (acc, line + 1)
  ...
  def main(args: Array[String]): Unit = {
    ...
    time("testSolve1", () => solve1(testData))
  ...
  private def solve1(implicit data: InputType): (Boolean, Int) = {
    val vm = new Vm
    vm.run
  }

  private def solve2(implicit data: InputType): (Boolean, Int) = {
    val vm = new Vm
    vm.debug
  }
