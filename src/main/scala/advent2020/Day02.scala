...
object Day02 {
  ...
  def solve1(policies: List[(Int, Int, Char, String)]): Int = {
    policies.count(t => {
      val cnt = t._4.count(_ == t._3)
      cnt >= t._1 && cnt <= t._2
    })
  }

  def solve2(policies: List[(Int, Int, Char, String)]): Int = {
    policies.count(t => {
      val a = t._4.charAt(t._1 - 1)
      val b = t._4.charAt(t._2 - 1)
      (t._3 == a || t._3 == b) && a != b
    })
  }

  def parse(inputs: String): List[(Int, Int, Char, String)] = {
    inputs.linesIterator
      .map(_.trim)
      .filter(_.nonEmpty)
      .map(_.split(Array('-', ' ')))
      .map(a => (a(0).toInt, a(1).toInt, a(2)(0), a(3)))
      .toList
  }

  def main(args: Array[String]): Unit = {
    val policies = parse(inputs)
    println(s"Inputs: ${policies.size}")
    println(s"Answer1: ${solve1(policies)}")
    println(s"Answer2: ${solve2(policies)}")
  }
}
